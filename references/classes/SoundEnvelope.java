public class SoundEnvelope {
	...
	final void decodeSegments(Buffer buffer) {
		...
		for (int segmentIndex = 0; segmentIndex < this.segments; ++segmentIndex) {
			this.durations[segmentIndex] = buffer.readUnsignedShort();
			this.phases[segmentIndex] = buffer.readUnsignedShort();
		}
	}
	...
	final int doStep(int sampleCount) {
		if (this.max >= this.ticks) {
			...
			this.ticks = (int)((double)this.durations[this.phaseIndex] / 65536.0D * (double)sampleCount);
