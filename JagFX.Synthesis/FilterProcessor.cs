...
public static class FilterProcessor
{
    ...
    public static void Apply(int[] buffer, Filter filter, EnvelopeEvaluator? envelopeEval, int sampleCount)
    {
        ...
        var tempInput = SampleBufferPool.Acquire(sampleCount);
    ...
    {
        var lastEnvelopeValue = Constants.FixedPoint.Scale;
        for (var i = start; i < end; i++)
        {
            ...
            lastEnvelopeValue = envelopeEval?.Evaluate(sampleCount) ?? Constants.FixedPoint.Scale;

            if (i + 1 < sampleCount)
            {
                var envelopeFactor = lastEnvelopeValue / (float)Constants.FixedPoint.Scale;
    ...
    private static float GetOctavePhase(float pow2Value)
    {
        ...
        return (float)(frequencyHz * MathUtils.TwoPi / Constants.SampleRate);
    }
    ...
    public class FilterState(Filter filter)
    {
        ...
        public int ComputeCoefs(int dir, float envelopeFactor)
        {
            ...
            InverseA0 = inverseA0;
            var floatInvA0 = inverseA0 / (float)Constants.FixedPoint.Scale;
        ...
        private int ComputeInverseA0(float envelopeFactor)
        {
            ...
            return (int)(floatInvA0 * Constants.FixedPoint.Scale);
        }
